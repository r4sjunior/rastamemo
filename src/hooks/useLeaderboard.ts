...
export function LeaderboardOnChain({ currentFid, onClose }: Props) {
  ...
  const { data: entries, isLoading, error } = useReadContract({
    ...
    // FIX: só executa se o contrato estiver deployado
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!CONTRACT_DEPLOYED && (
          <div style={{ fontFamily: F, fontSize: "7px", color: "#f59e0b", textAlign: "center", padding: "20px 0", lineHeight: 2.5 }}>
            ⚠️ Contrato não deployado ainda.{"\n"}
            Adicione NEXT_PUBLIC_LEADERBOARD_ADDRESS no .env
          </div>
        )}
